...
static ssize_t uts_status_proc_write(struct file *filp, const char __user *buff,
...
{
	uts_status_proc_file =
		proc_create(uts_status_PROC_FILE, 0666, NULL, &uts_status_fops);
...
static ssize_t get_usb_type_show(struct class *c, struct class_attribute *attr,
				 char *buf)
{
...
static ssize_t pm8350b_icl_store(struct class *c, struct class_attribute *attr,
				 const char *buf, size_t count)
{
	int rc;
	u32 tmp;
...
static ssize_t pm8350b_icl_show(struct class *c, struct class_attribute *attr,
				char *buf)
{
	int rc;
...
static ssize_t smb1396_icl_store(struct class *c, struct class_attribute *attr,
				 const char *buf, size_t count)
{
	int rc;
	u32 tmp;
...
static ssize_t smb1396_icl_show(struct class *c, struct class_attribute *attr,
				char *buf)
{
	int rc;
...
static ssize_t batt_FCC_store(struct class *c, struct class_attribute *attr,
			      const char *buf, size_t count)
{
	int rc;
	u32 tmp;
...
static ssize_t batt_FCC_show(struct class *c, struct class_attribute *attr,
			     char *buf)
{
	int rc;
...
static ssize_t smb_setting_store(struct class *c, struct class_attribute *attr,
				 const char *buf, size_t count)
{
	int rc;
	u32 tmp;
...
static ssize_t in_call_store(struct class *c, struct class_attribute *attr,
			     const char *buf, size_t count)
{
	int rc;
	u32 tmp;
...
static ssize_t in_call_show(struct class *c, struct class_attribute *attr,
			    char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%d\n", ChgPD_Info.in_call);
}
...
void asus_usb_thermal_worker(struct work_struct *work)
{
	...
	if (conn_temp > g_temp_Triger) {
		if (ChgPD_Info.usb_present) {
			...
			asus_extcon_set_state_sync(thermal_extcon,
	...
	CHG_DBG("conn_temp(%d), usb(%d), otg(%d), usb_connector = %d\n",
		conn_temp, ChgPD_Info.usb_present, ChgPD_Info.otg_enable,
		asus_extcon_get_state(thermal_extcon));
				...
				size_t len)
{
	...
	case OEM_SET_CHARGER_TYPE_CHANGE:
		if (len == sizeof(*Update_charger_type_msg)) {
			...
			CHG_DBG("%s OEM_SET_CHARGER_TYPE_CHANGE. new type : %d, old type : %d\n",
				__func__, Update_charger_type_msg->charger_type,
				pre_chg_type);
			if (Update_charger_type_msg->charger_type !=
			    pre_chg_type) {
				...
				case ASUS_CHARGER_TYPE_LEVEL1:
					g_SWITCH_LEVEL =
						SWITCH_LEVEL2_QUICK_CHARGING;
					break;
				case ASUS_CHARGER_TYPE_LEVEL2:
					g_SWITCH_LEVEL =
						SWITCH_LEVEL3_QUICK_CHARGING;
					break;
				case ASUS_CHARGER_TYPE_LEVEL3:
					g_SWITCH_LEVEL =
						SWITCH_LEVEL4_QUICK_CHARGING;
					break;
				default:
					g_SWITCH_LEVEL = SWITCH_LEVEL0_DEFAULT;
					break;
				...
				if (IS_ENABLED(
					    CONFIG_QTI_PMIC_GLINK_CLIENT_DEBUG) &&
				    qti_phy_bat)
					power_supply_changed(qti_phy_bat);
				cancel_delayed_work_sync(
					&asus_set_qc_state_work);
				if (g_Charger_mode) {
	...
	case OEM_ASUS_AdapterVID_REQ:
		if (len == sizeof(*adaptervid_msg)) {
			...
			asus_extcon_set_state_sync(adaptervid_extcon,
						   ChgPD_Info.AdapterVID);
		} else {
	...
	case OEM_OPCODE_READ_BUFFER:
		if (len == sizeof(*oem_read_buffer_resp_msg)) {
			...
			case BATTMAN_OEM_ADSP_PLATFORM_ID:
				ChgPD_Info.PlatformID =
					oem_read_buffer_resp_msg->data_buffer[0];
				ack_set = true;
				break;
			case BATTMAN_OEM_BATT_ID:
				ChgPD_Info.BATT_ID =
					oem_read_buffer_resp_msg->data_buffer[0];
				ack_set = true;
				if (ChgPD_Info.BATT_ID < 51000 * 1.15 &&
				    ChgPD_Info.BATT_ID > 51000 * 0.85)
					asus_extcon_set_state_sync(
						bat_id_extcon, 1);
				else if (ChgPD_Info.BATT_ID < 100000 * 1.15 &&
					 ChgPD_Info.BATT_ID > 100000 * 0.85)
					asus_extcon_set_state_sync(
						bat_id_extcon, 1);
				else
					asus_extcon_set_state_sync(
			...
			case BATTMAN_OEM_CHGPD_FW_VER:
				CHG_DBG("%s. ChgPD_FW : %s\n", __func__,
					(char *)oem_read_buffer_resp_msg
						->data_buffer);
				strcpy(ChgPD_Info.ChgPD_FW,
				       (char *)oem_read_buffer_resp_msg
					       ->data_buffer);
				ack_set = true;
				break;
...
static char *charging_stats[] = { "UNKNOWN", "CHARGING", "DISCHARGING",
				  "NOT_CHARGING", "FULL" };
char *health_type[] = { "UNKNOWN",
			"GOOD",
			"OVERHEAT",
			"DEAD",
			"OVERVOLTAGE",
...
static void print_battery_status(void)
{
	union power_supply_propval prop = {};
	char battInfo[256];
	int bat_cap, fcc, bat_vol, bat_cur, bat_temp, charge_status, bat_health,
		rc = 0;
	...
	rc = power_supply_get_property(
		qti_phy_bat, POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN, &prop);
	if (rc < 0) {
	...
	rc = power_supply_get_property(qti_phy_bat, POWER_SUPPLY_PROP_STATUS,
				       &prop);
	if (rc < 0) {
	...
	rc = power_supply_get_property(qti_phy_bat, POWER_SUPPLY_PROP_HEALTH,
				       &prop);
	if (rc < 0) {
	...
	snprintf(battInfo, sizeof(battInfo),
		 "report Capacity ==>%d, FCC:%dmAh, BMS:%d, V:%dmV, Cur:%dmA, ",
		 bat_cap, fcc / 1000, bat_cap, bat_vol / 1000, bat_cur / 1000);
	snprintf(battInfo, sizeof(battInfo),
		 "%sTemp:%d.%dC, BATID:%d, CHG_Status:%d(%s), BAT_HEALTH:%s \n",
		 battInfo, bat_temp / 10, bat_temp % 10, ChgPD_Info.BATT_ID,
		 charge_status, charging_stats[charge_status],
		 health_type[bat_health]);

	//ASUS_BSP +++ add to printk the WIFI hotspot & QXDM UTS event
	snprintf(UTSInfo, sizeof(UTSInfo), "WIFI_HS=%d, QXDM=%d", g_wifi_hs_en,
		 g_qxdm_en);
	//ASUS_BSP --- add to printk the WIFI hotspot & QXDM UTS event
...
void asus_set_qc_state_worker(struct work_struct *work)
{
	...
	CHG_DBG("%s: VID=%d, level=%d, boot=%d\n", __func__,
		ChgPD_Info.AdapterVID, g_SWITCH_LEVEL,
		ChgPD_Info.boot_completed);
	...
	CHG_DBG_E("%s: switch : %d, VID_changed=%d\n", __func__,
		  asus_extcon_get_state(quickchg_extcon), VID_changed);
}
...
void set_qc_stat(int status)
{
	...
	case POWER_SUPPLY_STATUS_FULL:
		cancel_delayed_work_sync(&asus_set_qc_state_work);
		if (g_Charger_mode) {
			schedule_delayed_work(&asus_set_qc_state_work,
					      msecs_to_jiffies(0));
		} else {
			schedule_delayed_work(&asus_set_qc_state_work,
	...
	default:
		cancel_delayed_work_sync(&asus_set_qc_state_work);
		asus_extcon_set_state_sync(quickchg_extcon,
					   SWITCH_LEVEL0_DEFAULT);
		break;
	}
}
//ASUS BSP : Show "+" on charging icon ---
...
void monitor_charging_enable(void)
{
	...
	if (g_Charger_mode) {
		CHG_DBG("%s. Charger mode battery protetction monitor",
			__func__);
		if (bat_capacity == 100 && !g_cos_over_full_flag) {
			g_charger_mode_full_time++;
			CHG_DBG("%s. Charger mode battery g_charger_mode_full_time  : %d",
				__func__, g_charger_mode_full_time);
			if (g_charger_mode_full_time >= g_ultra_cos_spec_time) {
				tmp = 1;
				ChgPD_Info.ultra_bat_life = tmp;
				tmp = ChgPD_Info.ultra_bat_life ||
				      ChgPD_Info.demo_app_status;
				CHG_DBG("%s. set BATTMAN_OEM_Batt_Protection : %d",
					__func__, tmp);

				rc = oem_prop_write(BATTMAN_OEM_Batt_Protection,
						    &tmp, 1);
				if (rc < 0) {
					pr_err("Failed to set BATTMAN_OEM_Batt_Protection rc=%d\n",
					       rc);
...
static ssize_t batt_safety_proc_write(struct file *file,
...
static ssize_t batt_health_config_write(struct file *file,
					const char __user *buffer, size_t count,
					loff_t *pos)
{
	int command = 0;
...
static ssize_t condition_value_proc_write(struct file *file,
					  ...
					  size_t count, loff_t *pos)
{
	...
	if (value && condition2_time <= condition1_time) {
		CHG_DBG_E(
			"[BAT][CHG]%s input value error,please input correct value!\n",
			__func__);
		return count;
	}
...
static int batt_safety_csc_restore(void)
{
	...
	if (rc < 0) {
		CHG_DBG_E(
			"[BAT][CHG]%s Read cycle count percent file failed!\n",
			__func__);
		return rc;
	}
	...
	if (rc < 0)
		CHG_DBG_E(
			"[BAT][CHG]%s Write cycle count percent file failed!\n",
			__func__);
...
static ssize_t batt_safety_csc_proc_write(struct file *file,
					  const char __user *buffer,
					  size_t count, loff_t *pos)
{
	int value = 0;
...
{
	struct proc_dir_entry *asus_batt_cycle_count_dir =
		proc_mkdir("Batt_Cycle_Count", NULL);
	struct proc_dir_entry *asus_batt_cycle_count_proc_file =
		proc_create("cycle_count", 0666, asus_batt_cycle_count_dir,
			    &cycle_count_fops);
	struct proc_dir_entry *asus_batt_batt_safety_proc_file =
		proc_create("batt_safety", 0666, asus_batt_cycle_count_dir,
			    &batt_safety_fops);
	struct proc_dir_entry *asus_batt_batt_safety_csc_proc_file =
		proc_create("batt_safety_csc", 0666, asus_batt_cycle_count_dir,
			    &batt_safety_csc_fops);
	struct proc_dir_entry *asus_batt_safety_condition_proc_file =
		proc_create("condition_value", 0666, asus_batt_cycle_count_dir,
			    &condition_value_fops);
	struct proc_dir_entry *batt_health_config_proc_file =
		proc_create("batt_health_config", 0666,
			    asus_batt_cycle_count_dir,
			    &batt_health_config_fops);
...
int asuslib_init(void)
{
	...
	if (g_ASUS_hwID <= HW_REV_EVB2) {
		POGO_OTG_GPIO = of_get_named_gpio(g_bcdev->dev->of_node,
						  "POGO_OTG_EN", 0);
	...
	if (IS_ERR(quickchg_extcon)) {
		...
		printk(KERN_ERR
		       "[BAT][CHG] failed to allocate ASUS quickchg extcon device rc=%d\n",
		       rc);
	}
	...
	if (rc < 0)
		printk(KERN_ERR
		       "[BAT][CHG] failed to register ASUS quickchg extcon device rc=%d\n",
		       rc);
	...
	if (IS_ERR(thermal_extcon)) {
		...
		printk(KERN_ERR
		       "[BAT][CHG] failed to allocate ASUS thermal alert extcon device rc=%d\n",
		       rc);
	}
	...
	if (rc < 0)
		printk(KERN_ERR
		       "[BAT][CHG] failed to register ASUS thermal alert extcon device rc=%d\n",
		       rc);
	...
	if (IS_ERR(adaptervid_extcon)) {
		...
		printk(KERN_ERR
		       "[BAT][CHG] failed to allocate ASUS adaptervid extcon device rc=%d\n",
		       rc);
	}
	...
	if (rc < 0)
		printk(KERN_ERR
		       "[BAT][CHG] failed to register ASUS adaptervid extcon device rc=%d\n",
		       rc);
	...
	INIT_DELAYED_WORK(&g_bcdev->update_gauge_status_work,
			  update_gauge_status_worker);
	schedule_delayed_work(&g_bcdev->update_gauge_status_work, 0);
	...
	if (g_Charger_mode) {
		...
		schedule_delayed_work(&asus_min_check_work,
				      msecs_to_jiffies(60000));
	}
