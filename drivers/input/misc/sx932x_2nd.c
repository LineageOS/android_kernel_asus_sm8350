...
#if 1
extern int asus_extcon_set_Capstate_sync(struct extcon_dev *edev, int cable_state); //ASUS_BSP
...
static int read_cap_status(psx93XX_t this)
{
	...
	if (ret < 0) {
		...
		cap2_info("Far");
		asus_extcon_set_Capstate_sync(this->cap_satus_extcon, 0);
		ret = 0;
	}else{
		cap2_info("Near");
		asus_extcon_set_Capstate_sync(this->cap_satus_extcon, 1);
		ret = 1;
	}
...
static void sx932x_reg_init(psx93XX_t this)
{
	...
	cap2_info("Setup I2C Registers done");
}
...
{    
	...
	cap2_info("%s start",__func__);
#if IS_ENABLED(CONFIG_ASUS)
	//If g_ASUS_capID=0 means HW is not support. 
	if(g_ASUS_capID == 0){
		cap2_info("HW is not support , Disable sx932x_2nd driver");
		return 0;
	}
#endif
