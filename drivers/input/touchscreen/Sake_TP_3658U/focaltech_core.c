...
*/
			if ((fp_press == 1) &&
			    (events[i].area >= data->fp_mini))
				fp_press = 0;

			if ((data->fp_report_type > 0) && (fp_press == 0)) {
				if (fp_position[0] <= events[i].x &&
				    events[i].x <= fp_position[1] &&
				    fp_position[2] <= events[i].y &&
				    events[i].y <= fp_position[3]) {
					FTS_DEBUG(
						"[B](id,x,y,p,area)(%d,%d,%d,%d,%d) DOWN!",
						events[i].id, events[i].x / 16,
						events[i].y / 16, events[i].p,
						events[i].area);
					if (events[i].area < data->fp_mini) {
						FTS_INFO(
							"Finger %d area %d too small , KEY O",
							events[i].id,
							events[i].area);
						input_report_key(
							data->input_dev, KEY_O,
							1);
						...
						FTS_INFO(
							"Finger %d down at FP area, KEY F",
							events[i].id);
						input_report_key(
							data->input_dev, KEY_F,
							1);
						input_sync(data->input_dev);
						input_report_key(
							data->input_dev, KEY_F,
					...
					FTS_INFO(
						"Finger %d down out of FP area,KEY L",
						events[i].id);
					input_report_key(data->input_dev, KEY_L,
							 1);
					input_sync(data->input_dev);
					input_report_key(data->input_dev, KEY_L,
							 0);
			...
			if ((data->log_level >= 2) ||
				...
				FTS_DEBUG(
					"[B](id,x,y,p,area,touchs)(%d,%d,%d,%d,%d,%d) DOWN!",
					events[i].id, events[i].x / 16,
					events[i].y / 16, events[i].p,
					events[i].area, touchs);
			}
		} else {
			if (!fts_data->wait_reset) {
				uppoint++;
...
static int fts_read_parse_touchdata(struct fts_ts_data *data)
{
	...
	for (i = 0; i < max_touch_num; i++) {
		...
		if (data->resize) {
			events[i].x =
				((buf[ASUS_TOUCH_X_1_POS + base] & 0x0F)
				 << 12) +
				((buf[ASUS_TOUCH_X_2_POS + base] & 0xFF) << 4);
			events[i].y =
				((buf[ASUS_TOUCH_Y_1_POS + base] & 0x0F)
				 << 12) +
		...
		events[i].p =
			buf[ASUS_TOUCH_Y_RATE_POS + base] & 0x0F; // report rate
	...
	if (data->touch_point == 0) {
		...
		FTS_INFO("no touch point information, pointid=%d, 0xa5=0x%02x",
			 pointid, val);
				 ...
				 unsigned long event, void *data)
{
	...
	case DRM_PANEL_BLANK_UNBLANK:
		...
		if (DRM_PANEL_EARLY_EVENT_BLANK == event) {
			...
			FTS_INFO("DRM_PANEL_EVENT_BLANK, Display on irq %d",
				 ts_data->irq_off);
			if (ts_data->irq_off == ENABLE) {
				...
				if (ts_data->next_resume_isaod) {
					FTS_INFO(
						"Display resume into AOD, not care");
				} else {
					if (fts_data->fp_report_type == 1) {
						FTS_INFO(
							"resume into hbm mode");
					} else {
						queue_work(
		...
		if (DRM_PANEL_EARLY_EVENT_BLANK == event) {
			...
			if (ts_data->phone_call_state == ENABLE &&
				...
				if (proxy_status) {
					...
					fts_irq_disable();
					FTS_INFO(
						"Proximity on and on phone call, disable irq not suspend");
					fts_release_all_finger();
				}
			}
			if (ts_data->irq_off != ENABLE) {
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
	...
	ts_data->fp_mini = 4;
	FTS_INFO("FOD location %d %d %d %d", fp_position[0], fp_position[1],
		 fp_position[2], fp_position[3]);
	return 0;
