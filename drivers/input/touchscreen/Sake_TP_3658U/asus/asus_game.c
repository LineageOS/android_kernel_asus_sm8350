...
u16 touch_level = 0x0, leave_level = 0x0, first_filter = 0x0,
    normal_filter = 0x0;
...
int set_rotation_mode(int angle)
{
	...
	if (ts_data->game_mode == ENABLE) {
		...
		case 90:
			...
			if (ret1 < 0 || ret2 < 0) {
				FTS_INFO(
					"Game mode enable, set rotation reg to 90 fail");
				ret = -1;
			} else
		...
		case 270:
			...
			if (ret1 < 0 || ret2 < 0) {
				FTS_INFO(
					"Game mode enable, set rotation reg to 270 fail");
				ret = -1;
			} else
				game_mode_active = true;
			break;
		case 0:
			if (game_mode_active) {
				ret1 = fts_write_reg(FTS_REG_EDGEPALM_MODE_EN,
						     0x00);
				ret2 = fts_write_reg(FTS_REG_CORNERPALM_AREA,
						     0x00);
				if (ret1 < 0 || ret2 < 0) {
					FTS_INFO(
						"Game mode enable, set rotation reg to 0 fail");
					ret = -1;
				} else
		...
		if (game_mode_active) {
			...
			if (ret1 < 0 || ret2 < 0) {
				FTS_INFO(
					"Game mode Exit, set rotation reg to 0 fail");
				ret = -1;
			} else {
				FTS_INFO(
					"Game mode Exit, set rotation reg to 0");
				game_mode_active = false;
			}
		} else
			FTS_INFO(
				"Not in game mode and never active corner protection, ignore rotation exit");
	}
...
void ATR_touch(int id, int action, int x, int y, int random)
{
	...
	FTS_INFO("keymapping ATR_touch  id=%d, action=%d, x=%d, y=%d", id,
		 action, x, y);
	...
	if (action) //press, find first slot or find last slot;
	{
		for (i = TOTAL_SLOT - 1; i >= 0; i--) {
			if (first_empty_slot == -1 &&
			    touch_figer_slot[i] == 0) //find first empty slot
				first_empty_slot = i;
			if (touch_figer_slot[i] ==
			    (id +
			     1)) //if the last id has been pressed, keep reporting same slot
				first_empty_slot = i;
		}
		FTS_INFO("keymapping ATR_touch press found slot %d",
			 first_empty_slot);
		if (first_empty_slot != -1) // found an available slot
		{
			if (touch_figer_slot[first_empty_slot] == 0)
				FTS_INFO(
					"keymapping report %d down x=%d ,y=%d ",
					first_empty_slot, x, y);
			...
			if (!fts_data->finger_press) {
				...
				FTS_INFO(
					"touch BTN down long pressed , ignore atr touch down");
			}
			input_sync(input_dev);

			touch_figer_slot[first_empty_slot] =
				id + 1; // save finger id in slot
		}
	} else //release
	{
		for (i = TOTAL_SLOT - 1; i >= 0; i--) {
			if (touch_figer_slot[i] ==
			    (id + 1)) //find the released slot
			{
				first_empty_slot = i;
				break;
			}
				      ...
				      const char *buf, size_t count)
{
	...
	FTS_INFO("keymapping ID=%d ACTION=%d X=%d Y=%d RANDOM=%d", id, action,
		 x, y, random);
...
static ssize_t fts_rotation_mode_show(struct device *dev,
				      struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d \n", fts_data->rotation_angle);
}

static ssize_t fts_rotation_mode_store(struct device *dev,
				       struct device_attribute *attr,
				       const char *buf, size_t count)
{
	if (buf[0] == '1') {
				   ...
				   const char *buf, size_t count)
{
	...
	FTS_INFO(
		"touch_level 0x%04X, leave_level 0x%04X first_filter 0x%04X, normal_filter 0x%04X",
		touch_level, leave_level, first_filter, normal_filter);
	FTS_INFO("Rcoefleft 0x%02X,RcoefRight 0x%02X touch_timer 0x%02X",
		 Rcoefleft, RcoefRight, touch_timer);

	return count;
}

static ssize_t fts_xy_resize_store(struct device *dev,
				   struct device_attribute *attr,
				   const char *buf, size_t count)
{
	struct fts_ts_data *ts_data = fts_data;
...
static DEVICE_ATTR(keymapping_touch, S_IRUGO | S_IWUSR, keymapping_touch_show,
		   keymapping_touch_store);
static DEVICE_ATTR(fts_game_mode, S_IRUGO | S_IWUSR, fts_game_mode_show,
		   fts_game_mode_store);
static DEVICE_ATTR(fts_rotation_mode, S_IRUGO | S_IWUSR, fts_rotation_mode_show,
		   fts_rotation_mode_store);
...
static struct attribute *fts_attributes[] = {
	&dev_attr_keymapping_touch.attr,  &dev_attr_fts_game_mode.attr,
	&dev_attr_fts_rotation_mode.attr, &dev_attr_game_settings.attr,
	&dev_attr_fts_xy_resize.attr,	  NULL
};
