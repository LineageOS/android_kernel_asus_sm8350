...
static int asus_ex_mode_switch(enum _asus_ex_mode mode, u8 value)
{
	...
	case MODE_CHARGER:
		if (!ts_data->power_disabled) {
			...
			FTS_ERROR(
				"TP regulator is disabled, skip write register.");
		}
		if (ret < 0) {
...
void fts_ex_fun_recovery(struct fts_ts_data *ts_data)
{
	...
	if (ts_data->charge_notify_charge) {
		FTS_INFO(
			"Charge mode enabled, receive usb plug message when suspend, restore charge mode");
		ret = asus_ex_mode_switch(MODE_CHARGER, ENABLE);
		if (ret >= 0) {
			ts_data->charger_mode = ENABLE;
		}
		...
		FTS_INFO(
			"Charge mode disabled, receive usb plug message when suspend, restore charge mode");
		ret = asus_ex_mode_switch(MODE_CHARGER, DISABLE);
		if (ret >= 0) {
			ts_data->charger_mode = DISABLE;
		}
 ...
 * Factory related
 */
static ssize_t fts_touch_status_show(struct device *dev,
 ...
 */
static ssize_t fts_fp_mode_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	int count = 0;
...
static ssize_t fts_fp_mode_store(struct device *dev,
...
static ssize_t fts_fp_ctrl_mode_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d \n", fts_data->fp_report_type);
}

static ssize_t fts_fp_ctrl_mode_store(struct device *dev,
				      ...
				      const char *buf, size_t count)
{
	...
	if (buf[0] == '0') {
		...
		if ((fts_data->display_state == 1) && fts_data->suspended) {
			FTS_INFO(
				"display on , touch not resume , resume touch");
			resume_touch(true);
		}
	} else if (buf[0] == '1') {
		...
		if (fts_data->next_resume_isaod) {
			...
			if (fts_data->auth_complete == 1) {
				FTS_INFO(
					"FP already auth success , reset fp_report_type to 0");
				fts_data->fp_report_type = 0;
				msleep(100);
			...
			if (fts_data->display_state == 1) {
				FTS_INFO(
					"FOD speedup ,FOD servicee request touch resume");
				fts_data->next_resume_isaod = false;
				if (fts_data->suspended) {
					FTS_INFO(
						"touch not resume , resume touch");
					resume_touch(true);
				}
			}
		}
...
static ssize_t fts_aod_ctrl_mode_show(struct device *dev,
				      struct device_attribute *attr, char *buf)
{
	int count = 0;
...
static ssize_t fts_aod_ctrl_mode_store(struct device *dev,
				       struct device_attribute *attr,
				       const char *buf, size_t count)
{
...
static ssize_t fp_auth_status_show(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d \n", fts_data->auth_complete);
}

static ssize_t fp_auth_status_store(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t count)
{
...
static ssize_t fp_area_show(struct device *dev, struct device_attribute *attr,
			    char *buf)
{
	return sprintf(buf, "%d \n", fts_data->fp_mini);
}

static ssize_t fp_area_store(struct device *dev, struct device_attribute *attr,
			     const char *buf, size_t count)
{
 ...
 * Gesture related
 */
static ssize_t fts_phonecall_state_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	int count = 0;
...
static ssize_t fts_phonecall_state_store(struct device *dev,
					 struct device_attribute *attr,
					 const char *buf, size_t count)
{
	struct fts_ts_data *ts_data = fts_data;
...
static ssize_t asus_ex_proc_glove_read(struct file *file, char __user *buf,
				       size_t count, loff_t *ppos)
{
	int len = 0;
	u8 val = 0;
...
static ssize_t asus_ex_proc_glove_write(struct file *filp, const char *buff,
...
static DEVICE_ATTR(touch_status, S_IRUGO | S_IWUSR, fts_touch_status_show,
		   NULL);
static DEVICE_ATTR(fts_fp_mode, S_IRUGO | S_IWUSR, fts_fp_mode_show,
		   fts_fp_mode_store);
static DEVICE_ATTR(fts_fp_ctrl_mode, S_IRUGO | S_IWUSR, fts_fp_ctrl_mode_show,
		   fts_fp_ctrl_mode_store);
static DEVICE_ATTR(fts_aod_ctrl_mode, S_IRUGO | S_IWUSR, fts_aod_ctrl_mode_show,
		   fts_aod_ctrl_mode_store);
static DEVICE_ATTR(fts_phone_state, S_IRUGO | S_IWUSR, fts_phonecall_state_show,
		   fts_phonecall_state_store);
static DEVICE_ATTR(fp_auth_status, S_IRUGO | S_IWUSR, fp_auth_status_show,
		   fp_auth_status_store);
static DEVICE_ATTR(fp_area, S_IRUGO | S_IWUSR, fp_area_show, fp_area_store);

/* add your attr in here*/
static struct attribute *fts_attributes[] = {
	&dev_attr_touch_status.attr,	 &dev_attr_fts_fp_mode.attr,
	&dev_attr_fts_fp_ctrl_mode.attr, &dev_attr_fts_aod_ctrl_mode.attr,
	&dev_attr_fts_phone_state.attr,	 &dev_attr_fp_auth_status.attr,
	&dev_attr_fp_area.attr,		 NULL
};

static struct attribute_group asus_attribute_group = { .attrs =
							       fts_attributes };
